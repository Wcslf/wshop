    ...
    protected function checkCategoryId($value, $rule, $data)
    {
        ...
        if ($data['id']) {
            ...
            if (count($diff) != 0) {//将之前有的id和现在没有的id提取出来查看下面是否有商品
                $is_goods = db('goods')->where('cat_id3','in',implode(',',$diff))->field('cat_id3')->find();
                if ($is_goods) {
            ...
            if (model('goods_category')->where('id', 'in', $value)->where("type_id<>0 && type_id<>{$data['id']}")->count() != 0) {
                $category_list = model('goods_category')->where('id', 'in', $value)->where("type_id<>0 && type_id<>{$data['id']}")->field('name')->select();
                $return_name = '';
                foreach ($category_list as $v){
                    $return_name .= '【'.$v->name.'】';
                }
                return $return_name.'分类已绑定';
            }
        }else{
            if (model('goods_category')->where('id', 'in', $value)->where("type_id<>0")->count() != 0) {
                $category_list = model('goods_category')->where('id', 'in', $value)->where("type_id<>0")->field('name')->select();
                $return_name = '';
                foreach ($category_list as $v){
                    $return_name .= '【'.$v->name.'】';
                }
