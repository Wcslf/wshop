...
class Report extends Base{
    ...
    public function saleTop(){
        $count = Db::name('order_goods'.$this->select_year)->where("is_send = 1")->group('goods_id')->count();
    ...
    // 兼容mysql5.7 sql_mode=only_full_group_by 错误问题             
        if(version_compare(mysql_version(),'5.7.0','>='))                
            $res = Db::name('order_goods'.$this->select_year)
                ->field('any_value(goods_name),any_value(goods_sn),sum(goods_num) as sale_num,sum(goods_num*goods_price) as sale_amount')
                ->where("is_send = 1")->group(' goods_id')->order('sale_amount DESC')->limit($Page->firstRow,$Page->listRows)->cache(true,3600)->select();
        else                            
            $res = Db::name('order_goods'.$this->select_year)
