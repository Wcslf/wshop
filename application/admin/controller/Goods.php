...
//                M('goods_category')->where("parent_id_path like '{$category['parent_id_path']}\_%'")->save(array('type_id' => $_POST['type_id'], 'commission' => $_POST['commission']));
    ...
    public function ajaxGoodsList(){ 
        ...
        ($is_on_sale !== '') && $where = "$where and is_on_sale = $is_on_sale";
        ($goods_state !== '') && $where = "$where and goods_state = $goods_state and is_on_sale<2";
        $cat_id = I('cat_id');
        // 关键词搜索               
...
//        dump($goods_category_list);exit;
    ...
    public  function addEditBrand(){        
            ...
            if(IS_POST)
            {
                ...
                //审核通过这个步骤，判断之前绑定在模型下的（这个品牌所绑定的分类）下面的品牌是否是全选，若是则自动将该品牌绑定到模型下
                if ($data['status'] == 0 && $id != 0) {
					
                    $cate_id = $data['cat_id1'];//品牌绑定的分类id
                    //查出该分类下面的所有品牌是否绑定了同一个模型
                    $is_not_select_brand = db('brand')
                        ->alias('b')
                        ->join('tp_brand_type t','b.id=t.brand_id','left')
                        ->where(['cat_id1'=>$cate_id])
                        //->where('b.id','<>',$id)
                        ->field('b.id,b.cat_id1,b.name,b.cat_name,t.type_id')
                        ->select();
					$brand_count = db('brand')->where(['cat_id1'=>$cate_id,'status'=>0])->where('id','<>',$id)->count();
					
						//dump($brand_count);
						//dump($is_not_select_brand);
					if(count($is_not_select_brand) > 0){
						$arr = [];
						foreach($is_not_select_brand as $k=>$v){
							if($v['type_id']){
								$arr[$v['type_id']][] = $v;
							}
						}
					...
					foreach($arr as $k=>$v){
						if(count($arr[$k]) == $brand_count){
							 db('brand_type')->add(['type_id'=>$k,'brand_id'=>$id]);
						}
					}
				
                   // if (count($is_not_select_brand) != 0 && !in_array('',array_column($is_not_select_brand,'type_id'))) {
                     //   db('brand_type')->add(['type_id'=>$is_not_select_brand[0]['type_id'],'brand_id'=>$id]);
                    //}
                }
    ...
    public function act()
    {
        ...
        if($act =='takeoff'){
        	 $goods = M('goods')->field('store_id,goods_name,goods_sn')->where(array('goods_id'=>$goods_ids))->find();
        	$takeoff_res=M('goods')->where(array('goods_id'=>$goods_ids))->save(array('is_on_sale' =>2,'goods_state'=>0,'close_reason'=>$reason));
            if($takeoff_res){
