    	...
    	foreach($withdrawals as $val){
			...
			/*code_8提现在线转账业务逻辑代码*/
    				if($val['bank_name'] == '支付宝'){
						...
						$message_logic->withdrawalsNotice($val['id'], $val['user_id'], $val['money']);
    					...
    					if($res['partner_trade_no']){
    						accountLog($val['user_id'], ($val['money'] * -1), 0,"平台理用户提现处申请");
    	...
    	foreach($withdrawals as $val){
			...
			/*code_8提现在线转账业务逻辑代码*/
    				if($val['bank_name'] == '支付宝'){
    					...
    					$wxpay = array(
    							'userid' => $val['user_id'],//用户ID做更新状态使用
    							'openid' => M("OauthUsers")->where(['user_id'=>$store['user_id'] , 'oauth_child'=>'mp'])->getField('openid'),//收款人微信号对应的 OPENID
