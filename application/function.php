...
function api_message($code)
{
    $arr = [
        "200" => ["code" => 200, "msg" => "成功", "msg_en" => "SUCCESS"],
        "101" => ["code" => 101, "msg"=>"请登录"],
        "102" => ["code" => 102, "msg"=>"用户不存在"],
    ];
    return $arr[$code];
}
/**
...
/**
 * 过滤属性空字符串, 将NULL设置为"", 防止APP端解析数据报错
 * @param unknown $model
 * @return unknown|string
 */
function filterNullAttribute($model){
 ...
 * @return string
 */
function getStoreScoreDec($score){
    if($score >= 4 || $score == 0){
        $dec = '高';
...
/**
 * 获取访问域名前缀,例如:bb2t.tpshop.com返回bb2t
 * @return Ambigous <>
 */
function get_secondary_prefix(){
    ...
    $master_domain = str_replace(["http://" , "https://"],"",$site_domain);
    
    //主域名就直接返回
    if($hostDomain == $master_domain)return 'MASTER';
    
    if($site_domain){
        preg_match("#(http://|https://)(.*?)\.#i",$site_domain,$matchs);
        //提商城域名,例如bb2t.tpshop.com 返回的是bb2t
        (count($matchs)>=2) && $masterPrefix = $matchs[2];
    }
   
     //如果二级域名标识是www或者是商城主域名则返回空
    if($hosts[0] == 'www' || $hosts[0] == $masterPrefix)  return null;
 ...
 * @return mixed
 */
function replaceSpecialStr($str , $replace=''){
