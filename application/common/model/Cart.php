...
class Cart extends Model
{
    ...
    {
        return $this->hasOne('PromGoods', 'id', 'prom_id')->cache(true,10)->field('id,title,description')->bind(['prom_title'	=> 'title','prom_detail'=>'description']);
    }

    public function goods()
    {
        return $this->hasOne('Goods', 'goods_id', 'goods_id')->cache(true,10)->field('goods_id,cat_id3,store_count,is_on_sale,prom_type,prom_id,weight')->bind([
            'cat_id3'	=> 'cat_id3','store_count'=>'store_count','is_on_sale'=>'is_on_sale','weight'=>'weight'
        ]);
    }
    ...
    {
        $total_fee = $data['goods_num'] * $data['goods_price'];
        return $total_fee;
    }
    /**
     * 商品总额优惠
    ...
    {
        $cut_fee = round(($data['goods_num'] * ($data['goods_price'] - $data['member_goods_price'])),2);
        return $cut_fee;
    }
