    ...
    public function getFinishedAttr($value, $data)
    {
        ...
        switch ($data['mmt_code']) {
            ...
            case 'prom_order_activity':
                // 订单促销
                $return_arr = Db::name('prom_order')->field('status,end_time')->where('id', $data['prom_id'])->find();
                if (time() > $return_arr['end_time'] or $return_arr['status'] == 0) {
                    $return_flag = true;
                }
                break;
            ...
            case 'pre_sell_activity':
                $return_arr = Db::name('pre_sell')->field('status,is_finished')->where('pre_sell_id', $data['prom_id'])->find();
                if (2 == $return_arr['is_finished'] or $return_arr['status'] != 1) {
                    $return_flag = true;
                }
                break;
            default:
    ...
    public function getStartTimeAttr($value, $data)
    {
        switch ($data['mmt_code']) {
            ...
            case 'team_activity':
                ...
                break;
            case 'pre_sell_activity':
                $return_arr = Db::name('pre_sell')->field('sell_start_time as start_time')->where('pre_sell_id', $data['prom_id'])->find();
                break;
            default:
                $return_flag = false;
                break;
    ...
    public function getGoodsIdAttr($value, $data)
    {
        ...
        switch ($data['mmt_code']) {
            ...
            case 'team_activity':
                ...
                break;
            case 'pre_sell_activity':
                $return_arr = Db::name('pre_sell')->field('goods_id,item_id')->where('pre_sell_id', $data['prom_id'])->find();
                break;
            default:
                $return_arr = ['goods_id'=>0, 'item_id'=>0];
                break;
    ...
    public function getHomeUrlAttr($value, $data)
    {
        switch ($data['mmt_code']) {
            ...
            case 'prom_order_activity':
                ...
                $uri = U("Home/Activity/promoteList", ['store_id'=>$data['store_id'],'type'=>'prom']);
                break;
            case 'combination_activity':
                // 搭配购 主商品
                $goods_id = Db::name('combination_goods')->where(['combination_id' => $data['prom_id'], 'is_master' => 1])->value('goods_id'); 
            ...
            case 'team_activity':
                ...
                $uri = U("Mobile/Team/info", ['goods_id' => $flash_sale['goods_id'], 'item_id' => $flash_sale['item_id']]);
                break;
            case 'pre_sell_activity':
                $flash_sale = Db::name('pre_sell')->field('goods_id,item_id')->where('pre_sell_id', $data['prom_id'])->find();
                $uri = U("Home/Goods/goodsInfo", ['id' => $flash_sale['goods_id'], 'item_id' => $flash_sale['item_id']]);
                break;
            default:
                $uri = '';
