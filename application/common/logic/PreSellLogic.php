...
class PreSellLogic extends Prom
{
    ...
    public function checkActivityIsAble(){
        ...
        if(time() > $this->preSell['sell_start_time'] && time() < $this->preSell['sell_end_time'] &&  $this->preSell['status'] == 1 && $this->preSell['is_finished'] == 0){
            return true;
        }
        return false;
    }
    ...
    public function buyNow($goods_num)
    {
        ...
        $cartInfo = [
            ...
            'store_id'=>$this->preSell['store_id'],
            'goods_num'=>$goods_num,
        ];
    ...
    {
        if($this->preSell){
            if($this->preSell['item_id']){
                $this->preSell->specGoodsPrice->save(['prom_type' => 0, 'prom_id' => 0]);
                $goodsPromCount = Db::name('spec_goods_price')->where('goods_id', $this->preSell['goods_id'])->where('prom_type','>',0)->count('item_id');
                if($goodsPromCount == 0){
                    $this->preSell->goods->save(['prom_type' => 0, 'prom_id' => 0]);
                }
            }else{
                $this->preSell->goods->save(['prom_type' => 0, 'prom_id' => 0]);
            }
            $this->preSell->sell_end_time = date('Y-m-d H:i:s');
            $this->preSell['is_finished'] = 1;
            $this->preSell['status'] = 3;
            $this->preSell->save();
        }
    }
