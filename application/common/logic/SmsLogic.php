...
class SmsLogic 
{
    ...
    public function sendSms($scene, $sender, $params, $unique_id=0)
    {
       ...
       if ($sender != '' && check_mobile($sender)) {//如果是正常的手机号码才发送
            try {
                $resp = $this->realSendSms($sender, $smsTemp['sms_sign'], $smsParam, $smsTemp['sms_tpl_code'],$smsTemp['tpl_content']);
            } catch (\Exception $e) {
    ...
    private function realSendSms($mobile, $smsSign, $smsParam, $templateCode,$tplContent = '')
    {
        ...
        switch($type) {
            ...
            case 2:
                ...
                break;
            case 3:
                $array = ['code','consignee','phone','orderId','userName','consignee','goodsName'];
                foreach ($smsParam as $k => $v){
                    if (in_array($k,$array)) {
                        $tplContent = str_replace('${'.$k.'}',$v,$tplContent);
                    }
                }
                $result = $this->sendSmsByLiuYiQuan($mobile,$tplContent);
                break;
            default:
    ...
    */
     ...
     * @param $templateCode
     */
    private function sendSmsByLiuYiQuan($mobile,$tplContent)
    {
        $accessKeyId = $this->config['sms_appkey'];
        $accessKeySecret = $this->config['sms_secretKey'];
        $url='http://120.24.178.250:7862/sms?';
        $post_data="action=send&account={$accessKeyId}&password={$accessKeySecret}&mobile={$mobile}&content={$tplContent}&extno=";
        $ch = curl_init();
        ...
        $objectxml = simplexml_load_string($file_contents);//将文件转换成 对象
        $xmljson= json_encode($objectxml );//将对象转换个JSON
        $xmlarray=json_decode($xmljson,true);//将json转换成数组

        if ($xmlarray['returnstatus'] == 'Success') {
            return array('status' => 1, 'msg' => '已发送成功, 请注意查收');
        } else {
            if($xmlarray['returnstatus'] == 'Failed'){
                return array('status' => -1, 'msg' => '请在后台配置短信或按照文档接入短信' .$xmlarray['message']);
            }else{
