...
class Message
{
    ...
    public function checkUserMessage($user_id)
    {
        ...
        if ($user_info) {
            $message_logic = new MessageBase([]);
            // 通知
            $message_id_array = Db::name('user_message')->where(array('user_id' => $user_info['user_id'], 'category' => 0))->column('message_id');
            $message_where = array(
                'message_type' => 1,
                'send_time' => array('gt', $user_info['reg_time']),
            );
            if (!empty($message_id_array)) {
                $message_where['message_id'] = array('NOT IN', $message_id_array);
            }
            ...
            foreach ($message_notice_no_read as $key) {
                if(!empty($key['store_id'])){
                    if($message_logic->hasCollectStore($user_info['user_id'], $key['store_id'])){
            ...
            $message_where = array(
                //'send_time' => array('gt', $user_info['reg_time']),
                'end_time' => array('gt', time()) // 活动没有结束
            );
            if (!empty($user_message)) {
                $user_id_array = get_arr_column($user_message, 'message_id');
                $message_where['message_id'] = array('NOT IN', $user_id_array);
            }
            $message_activity_no_read = Db::name('message_activity')->field('message_id,store_id')->order('send_time ASC')->where($message_where)->select();
            ...
            // 优惠券过期消息
            $messageLogic = new MessageNoticeLogic(['category' => 0]);
