...
class PromGoodsLogic extends Prom
{
    ...
    public function initProm()
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，活动和规格是一对多
            $this->promGoods = PromGoods::get($this->specGoodsPrice['prom_id'],'',true);
        }else{
            //活动商品没有规格，活动和规格是一对多
            $this->promGoods = PromGoods::get($this->goods['prom_id'],'',true);
        }
        if ($this->promGoods) {
    ...
    public function checkActivityIsAble(){
        ...
        if(time() > $this->promGoods['start_time'] && time() < $this->promGoods['end_time'] && $this->promGoods['status'] == 1 && $this->promGoods['is_end'] == 0){
            return true;
        }
        return false;
    }
    ...
    public function buyNow($buyGoods){
        if(!$this->checkActivityIsEnd() && $this->checkActivityIsAble()){
            if($buyGoods['goods_num'] > $this->promGoods['buy_limit']){
                throw new TpshopException('立即购买',0,['status' => 0, 'msg' => '每人限购'.$this->promGoods['buy_limit'].'件', 'result' => '']);
            }
        ...
        if($userBuyGoodsNum > $this->promGoods['buy_limit']){
            throw new TpshopException('立即购买',0,['status' => 0, 'msg' => '每人限购'.$this->promGoods['buy_limit'].'件，您已下单'.$userPromOrderGoodsNum.'件', 'result' => '']);
        }
        if($buyGoods['goods_num'] > $residue_buy_limit){  //不算购物车的
            throw new TpshopException('立即购买',0,['status' => 0, 'msg' => '商品库存不足，你只能购买'.$residue_buy_limit, 'result' => '']);
        }
