...
class FlashSaleLogic extends Prom
{
    ...
    public function initProm()
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->flashSale = FlashSale::get($this->specGoodsPrice['prom_id'],'',false);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->flashSale = FlashSale::get($this->goods['prom_id'],'',false);
        }
        if ($this->flashSale) {
    ...
    public function checkActivityIsAble(){
        ...
        if(time() > $this->flashSale['start_time'] && time() < $this->flashSale['end_time'] &&  $this->flashSale['status'] == 1 && $this->flashSale['is_end'] == 0){
            return true;
        }
        return false;
    }
    ...
    public function buyNow($buyGoods){
        if($this->checkActivityIsAble()){
            if($buyGoods['goods_num'] > $this->flashSale['buy_limit']){
                throw new TpshopException('立即购买',0,['status' => 0, 'msg' => '每人限购'.$this->flashSale['buy_limit'].'件', 'result' => '']);
            }
        }
        ...
        if($userBuyGoodsNum > $this->flashSale['buy_limit']){
            throw new TpshopException('立即购买',0,['status' => 0, 'msg' => '每人限购'.$this->flashSale['buy_limit'].'件，您已下单'.$userFlashOrderGoodsNum.'件', 'result' => '']);
        }
        $flashSalePurchase = $this->flashSale['goods_num'] - $this->flashSale['buy_num'];//抢购剩余库存
        if($buyGoods['goods_num'] > $flashSalePurchase){
            throw new TpshopException('立即购买',0,['status' => 0, 'msg' => '商品库存不足，剩余'.$flashSalePurchase, 'result' => '']);
        }
