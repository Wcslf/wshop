...
class Finance extends Base
{
    ...
    public function add_edit_withdrawals()
    {
        ...
        if (IS_POST) {
            ...
            if ($data['id']) {
                ...
                $withdrawal = Db::name('StoreWithdrawals')->where(array('store_id' => STORE_ID, 'status' => 0))->sum('money');
    ...
    public function order_list(){
        ...
        $order_sn ? $where['order_sn'] = $order_sn : false;
        $count = Db::name('order')->where($where)->count();
        $page = new Page($count,20);
        $lists = Db::name('order')->where($where)->select();
