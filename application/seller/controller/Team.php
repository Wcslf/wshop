...
class Team extends Base
{
	public function index()
	{
		...
		$count = $TeamActivity->where('store_id',STORE_ID)->where('deleted', 0)->count();
		$Page = new Page($count, 10);
		$show = $Page->show();
		$list = $TeamActivity->append(['team_type_desc','time_limit_hours','status_desc'])->where('deleted', 0)->where('store_id',STORE_ID)->order('add_time desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
