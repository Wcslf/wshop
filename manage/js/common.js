$(function(){
    ...
    if (!key) {
       // window.location.href = WapSiteUrl+'/tpl/mystore/login.html';
        return;
    }
});
...
function getFreeVoucher(e) {
    ...
    $.ajax({
        type: "post",
        url: ApiUrl + "/Membervoucher/voucher_freeex.html",
        data: {
            tid: e,
            key: t
        },
...
/*
 获取URL参数
 调用方法
 alert(GetUrlParameter("参数名1"));
 alert(GetUrlParameter("参数名2"));
 alert(GetUrlParameter("参数名3"));
 */
...
/*时间戳转换日期
 t:要转换的时间戳
 g:要获取的时间格式
 //调用方法<%= tsToDateString(v.addtime,'ymd','-')%>
 */
template.helper('tsToDateString', function (t, g) {
