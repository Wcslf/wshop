<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:51:"./template/mobile/default/index/ajaxStreetList.html";i:1532948114;}*/ ?>
            ...
            <div class="comment">
            	<ul>
            		...
            		<li>
						...
						<em>
							<?php if($store[store_servicecredit] == 0): ?>高<?php else: $_RANGE_VAR_=explode(',',"0.01,1.99");if($store['store_servicecredit']>= $_RANGE_VAR_[0] && $store['store_servicecredit']<= $_RANGE_VAR_[1]):?>低<?php endif; $_RANGE_VAR_=explode(',',"2,3.99");if($store['store_servicecredit']>= $_RANGE_VAR_[0] && $store['store_servicecredit']<= $_RANGE_VAR_[1]):?>中<?php endif; $_RANGE_VAR_=explode(',',"4,5");if($store['store_servicecredit']>= $_RANGE_VAR_[0] && $store['store_servicecredit']<= $_RANGE_VAR_[1]):?>高<?php endif; endif; ?>
						</em>
            		</li>
            		<li>
            			<span>物流速度</span>
            			...
            			<em>
							<?php if($store[store_deliverycredit] == 0): ?>高<?php else: $_RANGE_VAR_=explode(',',"0.01,1.99");if($store['store_deliverycredit']>= $_RANGE_VAR_[0] && $store['store_deliverycredit']<= $_RANGE_VAR_[1]):?>低<?php endif; $_RANGE_VAR_=explode(',',"2,3.99");if($store['store_deliverycredit']>= $_RANGE_VAR_[0] && $store['store_deliverycredit']<= $_RANGE_VAR_[1]):?>中<?php endif; $_RANGE_VAR_=explode(',',"4,5");if($store['store_deliverycredit']>= $_RANGE_VAR_[0] && $store['store_deliverycredit']<= $_RANGE_VAR_[1]):?>高<?php endif; endif; ?>
						</em>
            		</li>
            	</ul>
            </div>

            <?php if(!(empty($store['goods_list']) || (($store['goods_list'] instanceof \think\Collection || $store['goods_list'] instanceof \think\Paginator ) && $store['goods_list']->isEmpty()))): ?>
            <div class="baokaun">
				<div class="shop">
					<ul>
					<?php if(is_array($store['goods_list']) || $store['goods_list'] instanceof \think\Collection || $store['goods_list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $store['goods_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>
						<li>
