...
{    
    ...
    /*code_8提现在线转账业务逻辑代码*/
    	...
    	$parameter = array(
    			...
    			"notify_url" => SITE_URL.U('Home/Payment/transferBack',array('pay_code'=>'alipay')),//回调通知地址
    			"email"	=> $this->alipay_config['seller_email'],//付款账号
    			"account_name"	=> $this->alipay_config['alipay_account_name'],//付款方的支付宝账户名
    ...
    /*code_8提现在线转账业务逻辑代码*/
    	...
    	if($verify_result){
    		...
    		if(stripos($batch_no,'u') !== false){
    			$tb = 'withdrawals';
    		}else{
    			$tb = 'store_withdrawals';
    		}
    		...
    		if($success_details){
    			...
    			$withdrawals = M($tb)->where(array('id'=>array('in',$pay_id)))->select();
    			foreach ($withdrawals as $wd){
    				if($tb == 'withdrawals'){
    					$expense_data = array('money'=>$wd['money'],'log_type_id'=>$wd['id'],'type'=>1,'user_id'=>$wd['user_id']);
    					accountLog($wd['user_id'], ($wd['money'] * -1), 0,"平台管理员处理用户提现申请");
    				}else{
    					$expense_data = array('money'=>$wd['money'],'log_type_id'=>$wd['id'],'type'=>0,'store_id'=>$wd['store_id']);
    					storeAccountLog($wd['store_id'], ($wd['money'] * -1), 0,"平台管理员处理商家提现申请");
    				}
    				expenseLog($expense_data);//平台支出记录
    			}
    			M($tb)->where(array('id'=>array('in',$pay_id)))->save(array('pay_time'=>strtotime($pay_arr[7]),'status'=>2,'pay_code'=>$pay_arr[6]));
    		}else{
    			...
    			foreach ($fdata as $val){
    				...
    				M($tb)->where(array('id'=>$pay_arr[0]))->save($update);
    			}
    		}
    		echo "success"; //告诉支付宝处理成功
    	}else{
    ...
    /*code_4支付原路退回逻辑*/
    	...
    	$parameter = array(
    			...
    			"notify_url" => SITE_URL.U('Home/Payment/refundBack',array('pay_code'=>'alipay')),
