            ...
            if ($httpClient->call()) {
                ...
                if ($resHandler->getParameter("trade_mode") == "1") {
					...
					if (stripos($out_trade_no, 'recharge') !== false)
		                ...
		                $order_amount = M('order')->where(['master_order_sn'=>"$out_trade_no"])->whereOr(['order_sn'=>"$out_trade_no"])->sum('order_amount');
			               if((string)($order_amount * 100) != (string)$total_fee) 
				              exit('fail'); //验证失败                               
                        
                        update_pay_status($out_trade_no,$transaction_id); // 修改订单支付状态
                        //------------------------------
                        //处理业务完毕
                        //------------------------------
