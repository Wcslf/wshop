...
EOF;
    ...
    /*code_8提现在线转账业务逻辑代码*/
        ...
        $cert_path= $ssl_path_arr[WxPayConfig::$app_type]['ssl_cert'];
        $key_path= $ssl_path_arr[WxPayConfig::$app_type]['ssl_key'];
        
        $wxchat['appid'] = WxPayConfig::$appid;
        $wxchat['mchid'] = WxPayConfig::$mchid;
        $wxchat['api_cert'] =$cert_path;
        $wxchat['api_key'] = $key_path;
    	...
    	sort($tarr);
    	$sign = implode($tarr, '&');
    	$sign .= '&key='.WxPayConfig::$key;//服务商支付秘钥
